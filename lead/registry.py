...
class AgentRegistry:
    ...
    def _load_or_rehydrate(self) -> None:
        if self.index_path.exists():
            try:
                ...
                self._records = {
                    aid: AgentRecord(agent_id=aid, **fields) for aid, fields in data.items()
                }
                return
    ...
    def save(self) -> None:
        data = {
            aid: {k: v for k, v in asdict(r).items() if k != "agent_id"}
    ...
    def update(self, agent_id: str, **changes: object) -> AgentRecord:
